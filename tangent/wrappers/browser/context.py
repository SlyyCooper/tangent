...
from tangent.wrappers.browser.views import BrowserError, BrowserState, TabInfo
from tangent.wrappers.dom.service import DomService
from tangent.wrappers.dom.views import DOMElementNode, SelectorMap
from tangent.wrappers.utils import time_execution_sync
...
class BrowserContext:
	...
	async def get_state(self, use_vision: bool = True) -> BrowserState:
		...
		Vision is enabled by default for richer context and better agent understanding.
		"""
	...
	async def _update_state(self, use_vision: bool = True) -> BrowserState:
		"""Update and return state with visual context by default."""
		session = await self.get_session()
		...
		try:
			# Remove any existing highlights before capturing state
			await self.remove_highlights()
			
			# Get DOM state and screenshot concurrently for better performance
			dom_service = DomService(page)
			content_task = dom_service.get_clickable_elements()
			screenshot_task = self.take_screenshot() if use_vision else None
			
			# Gather results
			content = await content_task
			screenshot_b64 = await screenshot_task if screenshot_task else None
	...
	async def take_screenshot(self, full_page: bool = False) -> str:
		...
		Optimized for vision processing with disabled animations and compressed output.
		"""
		page = await self.get_current_page()
		...
		try:
			...
			screenshot = await page.screenshot(
				full_page=full_page,
				animations='disabled',
				type='jpeg',  # Use JPEG for smaller size
