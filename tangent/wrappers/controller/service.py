...
from tangent.wrappers.controller.registry.service import Registry
from tangent.wrappers.controller.views import (
	ActionResult,
	ClickElementAction,
	DoneAction,
...
from tangent.wrappers.utils import time_execution_async, time_execution_sync
...
class Controller:
	...
	def _register_default_actions(self):
		...
		async def search_google(params: SearchGoogleAction, browser: BrowserContext):
			...
			logger.info(msg)
			return ActionResult.create(content=msg, include_in_memory=True)
		...
		async def click_element(params: ClickElementAction, browser: BrowserContext):
			...
			try:
				...
				return ActionResult.create(error=str(e))
		...
		async def extract_content(params: ExtractPageContentAction, browser: BrowserContext):
			...
			logger.info(msg)
			return ActionResult.create(content=msg)

		@self.registry.action('Complete task', param_model=DoneAction)
		async def done(params: DoneAction):
			return ActionResult.create(content=params.text, is_done=True)
		...
		async def scroll_to_text(text: str, browser: BrowserContext):  # type: ignore
			...
			try:
				...
				return ActionResult.create(
					error=f'Could not find text "{text}" on page'
				)
			except Exception as e:
				return ActionResult.create(error=str(e))
		...
		async def select_dropdown_option(
			...
			try:
				...
				logger.error(msg)
				return ActionResult.create(error=msg, include_in_memory=True)
			...
			for action_name, params in action.model_dump(exclude_unset=True).items():
				if params is not None:
					...
					if isinstance(result, str):
						return ActionResult.create(content=result)
